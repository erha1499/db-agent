...
- 维度和指标按列位置选择，重名列仍分别保留。分组以返回值精确匹配；不会模拟数据库 collation 的大小写、重音或尾随空格等行为。NULL 维度与字符串 `"NULL"` 不合并。
...
新数据源保留同样的身份/会话/运行/结果标识核对、原始SQL范围和精度要求。
PostgreSQL `timestamp` 不带时区，`timestamptz` 按已验证的UTC会话返回；
两者均可作为趋势时间列，`timestamptz` 仅接受完整ISO格式的UTC值。
分组仍按完全相同的返回时间值，不隐式按天聚合，不合并不同时间点。
日期/金额标签和导出保留原值；文字说明不把PostgreSQL的无时区timestamp说成UTC。
跨数据源/schema/身份/授权代际的旧结果不可取回。实际新源验收见
[PostgreSQL说明](postgresql.md)。

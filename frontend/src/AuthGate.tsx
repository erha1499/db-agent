...
export default function AuthGate({
  ...
  children: (identity: Identity, boundary: string, logout: () => void) => ReactNode;
}) {
  ...
  const verify = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        clearAccess('登录已失效或身份权限已变更，请重新登录。');
        return;
      }
      ...
      clearAccess(`无法核对登录状态：${message(cause)}`);
    } finally {
  ...
  if (session?.authenticated && session.identity)
    return children(session.identity, boundary, () => void logout());

  return (
    <main className="auth-page">
      <section className="auth-card" aria-label="登录本机工作区">
        ...
        <h1>登录本机工作区</h1>
        <p className="muted">本机隔离合成环境。使用管理员配置的身份访问获准数据。</p>
        {checking ? (
        ...
        <div className="auth-boundary">
          ...
          <p>{boundary}</p>
          <p>请勿在智能查询中输入不允许发给该模型的原文。权限与模型可用表范围由服务端管理。</p>
        </div>
      </section>
    </main>
  );

...
**当前阶段：核心查询闭环与原 1–19 步已交付各自限定范围，Web 默认免登录并自动准备页面。** 状态核对至 2026-09-11；启动命令见 [README](README.md)，开发约定见 [AGENTS.md](AGENTS.md)。数据库配置和合成数据初始化仍需显式完成；只用 SQL 不需要模型配置。

## 本轮简化

- [x] `uv run db-agent web` 默认打开本机工作区，无需用户管理、用户名或密码；表范围来自现有数据库白名单，模型配置完成后即可使用智能查询。
- [x] 启动自动检查页面产物，缺失或源码变化时构建；依赖未安装或清单变化时按锁文件安装，日常启动复用产物。
- [x] 更新首次和日常启动说明；原多身份隔离保留为显式 `--identity-file` 的可选模式，库存变更仍需独立配置和审批权限。
- [x] 修复“帮我查询最近的10个订单”生成合法 SQL 后被默认明细口径误拦：三阶段共享常规明细默认，保留独立合同、最终复核与执行重检；[规则说明](docs/semantic-review.md)。

启动分支验证见 `tests/test_web_startup.py`；本轮 Python 1986 项通过、200 项显式集成跳过，真实 MySQL 44 项和浏览器原问题均通过，详细命令及证据边界见[本轮验收](docs/web.md#2026-09-11-免登录与启动简化验收)，与下方历史数字分别判断。
...
| 优先级 | 待办 | 完成时应能验证什么 |
| --- | --- | --- |
| P1 | **集中数据库配置与启动自检**：报告缺少配置、数据库连接、表范围与端口问题 | 错误能直接定位到修复步骤；不打印凭据；SQL 检查不依赖模型，模型连通性需显式触发 |
| P2 | **自动核对 README 的首次与再次启动流程** | 隔离合成环境按文档完成安装、显式初始化、查询、停止后重启；重复操作保留已有数据；真实模型验证单独记录 |
